# PhotoSignature MongoDB Schema v6

> ⚠️ **v7로 대체됨. MONGODB_SCHEMA_V7.md 참조**

> **Created:** 2025-12-10
> **Project:** PhotoSignature New System
> **Version:** 6.0 (v5 Review Applied)
  ...
  validator: {
    $jsonSchema: {
      ...
      properties: {
        ...
        // v6: Added discount validation to prevent negative values
        discount: {
          bsonType: "object",
          properties: {
...
```javascript
{
  ...
  // Using Decimal128 for financial precision (see "Precision Note" section)
  ...
  // v6: Original data snapshot (stored only on cancel/refund)
  cancelSnapshot: null,                     // { originalAmount, originalAmountKRW, originalStatus }
...
**Indexes:**
```javascript
// v6: Minimized indexes for write-heavy workload (14 → 8)
...
- `amountKRW` - Converted KRW amount
- `exchangeRate` - Exchange rate at transaction time
- `cancelSnapshot.originalAmount` - Original amount before cancel
- `cancelSnapshot.originalAmountKRW` - Original KRW amount before cancel
...
### Total Index Count: 8 (v6: minimized for write-heavy workload)
...
| v5 | v6 | Reason |
|----|----|--------|
| 15 indexes | 8 indexes | Minimized for write-heavy workload |
| Exchange rate cache 7 days | Exchange rate cache 3 days | Minimize exchange rate fluctuation risk |
| `Number` type for amounts | `Decimal128` type | Financial precision (USD has 2 decimal places) |
| - | `cancelSnapshot` field added | Preserve original data before cancel |
| - | `discount` negative validation added | Data integrity |
