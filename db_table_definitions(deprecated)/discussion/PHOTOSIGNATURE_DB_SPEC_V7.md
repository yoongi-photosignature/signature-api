# PhotoSignature MongoDB Schema v7

> **Created:** 2025-12-10
> **Project:** PhotoSignature New System
> **Version:** 7.0 (v6 Review Applied)
...
```javascript
{
  ...
  // v7: Original data snapshot (stored only on refund)
  // Note: FAILED transactions don't need snapshot (payment never completed)
  refundSnapshot: null,                     // { originalAmount, originalAmountKRW }
  ...
  popup: {
    ...
    revenue: {
      ...
      corpRate: 0.50,
      licenseRate: 0.20,
      // Calculated amounts (v7: Decimal128)
      storeAmount: NumberDecimal("1500"),
      corpAmount: NumberDecimal("2500"),
      licenseAmount: NumberDecimal("1000")
    }
  },

  // Service usage (v7: All fees Decimal128)
  services: {
    beauty: { used: true, fee: NumberDecimal("500") },
    ar: { used: false, fee: NumberDecimal("0") }
  },
...
```javascript
{
  ...
  // Pricing (can vary per popup) - v7: All Decimal128
  pricing: {
...
### 6. Failed/Refunded Transactions
...
**Decimal128 Fields (v7: Comprehensive list):**
...
| sales | `popup.revenue.storeAmount` | Store revenue share |
| sales | `popup.revenue.corpAmount` | Corp revenue share |
| sales | `popup.revenue.licenseAmount` | License revenue share |
| sales | `services.beauty.fee` | Beauty service fee |
| sales | `services.ar.fee` | AR service fee |
...
| v6 | v7 | Reason |
|----|----|--------|
| `status: "CANCELLED"` | `status: "FAILED"` | Clarify: payment failure vs refund |
| `cancelledAt`, `cancelReason`, `cancelledBy` | `failedAt`/`failReason` or `refundedAt`/`refundReason`/`refundedBy` | Separate failure and refund handling |
| `cancelSnapshot` | `refundSnapshot` | Only needed for refunds (FAILED has no completed payment) |
| `discount.roulette: number` | `discount.roulette: Decimal128` | All monetary fields use Decimal128 |
| - | `stores → devices` sync | Data consistency mechanism added |
| - | Firestore `deletedAt` | Soft delete for frames (documented) |
